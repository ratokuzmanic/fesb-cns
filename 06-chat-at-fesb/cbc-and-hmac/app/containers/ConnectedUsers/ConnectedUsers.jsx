...
const ConnectedUsers = ({ clients, credentials, ui }) => {
    let decoratedClients = <div className='ConnectedUsers__shrug'>¯\_(ツ)_/¯</div>
        ...
        decoratedClients = Object.keys(clients).map((key) => {
            ...
            return (
                <div key={key} className='ConnectedUsers__row'>
                    <DecoratedLetter 
                        color={ui[key]['color']} 
                        letter={clients[key][0]} 
                    />

                    <div className='ConnectedUsers__nickname-cell'>
                        {clients[key]}
                    </div>
                    <div>
                        <Lock open={lockOpen} /> 
    ...
    return (
        <div className='ConnectedUsers'>
            <div className='ConnectedUsers__header'>
                Connected users:
            </div>
            {decoratedClients}
        </div>
...
const ConnectedUsersContainer = connect(
    ({ clients, credentials, ui }) => ({ clients, credentials, ui })
)(ConnectedUsers)
