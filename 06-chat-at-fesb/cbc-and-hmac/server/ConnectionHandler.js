...
class ConnectionHandler {
    ...
    handleData(data) {
        ...
        if (server.isRegistered(id)) {
            switch(msg.type) {
                ...
                default:
                    return log.error(null, 
                        `Unknown, missing or wrong message type ${data} [${this.remoteClient}]`
                    )
            }
        }
        ...
        if (Object.is(data, undefined)) {
            return log.error(
                error, `Error in ${handleData.name} [${this.remoteClient}]`
            )
        }
     ...
     */
    handleClose() {        
        log.important(`Closing a socket with ${this.remoteClient} [${this.socket.id}]`);
        this.handleEnd()
    }
    ...
    handleEnd() {
        ...
        if (Object.is(msg, undefined)) {
            return log.error(
                error, `Error in ${handleSocketEnd.name} [${this.remoteClient}]`
            )        
        }
