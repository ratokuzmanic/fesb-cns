...
decryptChallenge = (sharedDiffieHellmanKey, challenge) =>
    new Promise((resolve, reject) => {
        crypto.pbkdf2(sharedDiffieHellmanKey, pbkdf2.salt, pbkdf2.iterations, pbkdf2.size, pbkdf2.hash, (error, key) =>
            error
            ? reject(`Failed to generate a key with error: ${error}`)
            : resolve(decrypt('aes-256-ctr', key, challenge.iv, challenge.ciphertext))
        )
    });
